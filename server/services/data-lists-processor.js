...
var playersRepo = require('../repositories/players-repository')
...
function DataListsProcessor(){
    this.update = async function(){
        ...
        await playersRepo.deleteAll();

        var iteration = 0;
        dataArr.forEach(async function(playersDataPage){
            console.log("iteration " + iteration);
            populatePlayersData(playersDataPage.items, data);

            await playersRepo.saveMany(playersDataPage.items);
            iteration++;
        });
